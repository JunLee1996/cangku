...
namespace WebApplication4.Controllers
{
    public class InvoicingController : Controller
    {
        ...
        public ActionResult InvoicingModification()
        {
                ...
                ViewBag.date = Request["InDate"];
                string s9 = Request["ID"];
                ObservableCollection<Invoicing> oss = SqlQuery.InvoicingByID(new Guid(s9));
                Session["Inid"] = Request["ID"];
                decimal d = Convert.ToDecimal(Request["log"]);
                ViewBag.ss1 = oct[0].Contract_Amount;
                ViewBag.service = oss[0].ServiceID;
                return View();
            
        }
        public ActionResult ChangeInvoicingLog(Invoicing inc)
        {
                ...
                ViewBag.p = Session["Inid"];
                Session.Remove("Inid");
                string sss = ViewBag.p;
                Guid ID2 = new Guid(sss);
                ObservableCollection < Invoicing > oig = SqlQuery.InvoicingDateByID(ID2);
                inc.ID =ID2;
                inc.LogDate =oig[0].LogDate;
                inc.Contract_ID = ID;
                ObservableCollection<Contract_Data> odd = SqlQuery.Contract_DataByIDQuery(inc.ServiceID);
                inc.Service = odd[0].Service;
               
                ObservableCollection<Accountant> oc = SqlQuery.AccountantByServiceQuery(inc.ServiceID);
                oc[0].SubInvoiceCount = inc.Count-oig[0].Count;
                oc[0].SubInvoiceAmount = inc.Amount - oig[0].Amount;
                oc[0].InvoicingDate = inc.InvoicingDate;
                SqlQuery.updataAcc(oc[0]);
                SqlQuery.updata(inc);
                return RedirectToAction("Invoicing");
